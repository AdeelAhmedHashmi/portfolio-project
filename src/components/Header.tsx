    ...
    <div className="px-4 w-screen lg:pt-14 pt-9 sm:px-8 md:px-16 lg:px-24 xl:px-23 font-sans">
      <AnimatePresence>
        {isOpen && (
          ...
          >
            <div
              className="absolute inset-6 text-2xl"
              onClick={() => setIsOpen(false)}
            >
              <RxCross1 />
            </div>
            <ul className="flex h-full flex-col justify-center w-screen *:text-xl *:p-1 *:hover:bg-primary gap-4 align-middle text-center ">
              <Navigation />
            </ul>
          </motion.div>
        )}
